...
- Pseudo-randomly samples a per-job validator committee from the active staker set with owner-managed 1-100 reputation weights, excludes the job's client/provider/evaluator from that draw, filters out validators whose current stake cannot cover the job's slashable validator-reward requirement, reserves each selected validator's slashable stake immediately, downranks repeated no-shows via assignment/response-aware selection weights, defaults unset validators to the legacy max score, exposes the selected panel onchain, snapshots each selected validator's draw-time weight inputs for later audit, and rejects votes from non-selected validators.
...
- Keeps low-reputation or inactive jurors out of new panels while still exposing the selected panel and each juror's draw-time selection snapshot for offchain transparency and auditability.
- Blends owner-set juror baselines with onchain appeal accuracy, panel-response history, and current pending-panel load so repeat no-shows, misaligned jurors, and already-busy jurors get downweighted in future panel draws without changing the minimum baseline gate.
...
`test/CommitteeReviewEvaluator.t.sol` covers committee approval and rejection flows, reputation- and response-weighted sampled-committee membership enforcement, draw-time committee snapshot persistence, stake-coverage-aware committee capacity checks, selected-validator unstake locks until accounting finalization, dispute-window gating, jury/dispute overrides of validator accounting, expired-appeal no-fault accounting, reward splitting across aligned validators, opt-params hash binding, performance-derived validator reputation updates, no-show penalties in future selection, and slashing after three consecutive deviations.

`test/HumanJury.t.sol` covers high-reputation jury-panel selection, low-reputation juror exclusion, draw-time jury snapshot persistence, upheld-vs-rejected dispute outcomes, jury reward distribution, no-contest performance accounting on expired reviews, response/accuracy-weighted future panel sampling, and the bridge from committee review into final jury accounting.
