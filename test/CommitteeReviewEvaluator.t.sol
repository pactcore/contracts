...
contract CommitteeReviewEvaluatorTest is Test {
    ...
    function setUp() external {
        ...
        committeeEvaluator = new CommitteeReviewEvaluator(
            address(commerce), address(usdc), MINIMUM_STAKE, SLASHING_BPS, SLASH_AFTER_DISAGREEMENTS, treasury
        );
    ...
    function testCommitteeApprovalsCompleteJobAndSplitValidatorReward() external {
        ...
        _assertValidatorAccount(validatorA, MINIMUM_STAKE, validatorReward / 2, 0, true);
        _assertValidatorAccount(validatorB, MINIMUM_STAKE, validatorReward / 2, 0, true);
        _assertValidatorAccount(validatorC, MINIMUM_STAKE, 0, 0, true);
    ...
    function testCommitteeRejectionsRefundClientAndDoNotMintRewards() external {
        ...
        (, uint256 rewardsA,,) = committeeEvaluator.validators(validatorA);
        (, uint256 rewardsB,,) = committeeEvaluator.validators(validatorB);
        assertEq(rewardsA, 0);
        assertEq(rewardsB, 0);
    }
    ...
    function testCommitteeSlashesValidatorAfterThreeConsecutiveDeviations() external {
        ...
        (uint256 stakeC, uint256 rewardsC, uint8 deviationsC, bool activeC) = committeeEvaluator.validators(validatorC);
        assertEq(stakeC, MINIMUM_STAKE - slashAmount);
        assertEq(rewardsC, 0);
        assertEq(deviationsC, 0);
        assertFalse(activeC);
    }
    ...
    function _assertValidatorAccount(
        ...
        (uint256 stakeAmount, uint256 rewardAmount, uint8 deviationCount, bool active) =
